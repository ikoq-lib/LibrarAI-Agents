...
- LDR, 008, 020, 040, 049, 056, 090, 245, 260, 300, 650/653, 700/710, 950 등 필수 필드가 모두 존재하는가
- **고정값 표기가 정확한가(배치 내 표기 흔들림 차단):** LDR이 `00000nam a2200000 c 4500`과 문자 단위로 일치(24자), `001`이 `ctrl_no` 0채움 12자리, `008`이 40자리이고 `29-31`이 `000`(색인 수록 시 `001`), `049`에 `$c1`이 없음(1부 입고면 `$c` 생략), 세트 ISBN이 `$a<ISBN> (세트)`(괄호 앞 공백 1칸), 어떤 필드도 마침표로 끝나지 않음. **한 배치 안에서 이 값들이 레코드마다 다르면 QA 실패로 처리한다** — 개별 레코드로는 그럴듯해 보여도 배치 전체의 일관성이 깨진다
