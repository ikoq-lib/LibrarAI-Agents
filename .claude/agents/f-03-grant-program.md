...
description: "Tracks and supports applications for external 공모사업. Registers grants the librarian received by 공문 from a supervising authority (immediately, no search step) or discovers them via proactive web search. Analyzes eligibility, tracks D-7 and D-1 deadlines, drafts the application and 사업계획서 via A-01, records outcomes, and maintains the annual history with re-application candidates. On selection, hands the confirmed amount to A-03 for budget reflection. Web-discovered candidates are registered only after librarian confirmation. Does not handle 협력기관, 의원, or 운영위원회 work (F-02's job)."
model: sonnet
color: orange
memory: project
---
