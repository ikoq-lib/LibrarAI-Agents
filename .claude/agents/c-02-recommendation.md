...
description: "The only agent that answers an individual patron's on-the-spot book request at the kiosk — general (요즘 인기 있는 책), personalized by member number or a recently borrowed title, and topic or mood based (힐링되는 소설). C-01 routes personalized-recommendation questions here. Falls back to popular and new-arrival recommendations from the last 3 months of loan statistics when the patron cannot be identified. Also manages the librarian's periodic loan-statistics Excel upload that backs all recommendation modes. Does not do program 북큐레이션 (D-02), reading-club session picks (D-01), or collection-wide KDC balance analysis (B-05)."
model: sonnet
color: purple
memory: project
---
