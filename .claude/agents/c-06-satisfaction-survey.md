...
description: "Collects and aggregates patron satisfaction data: the always-on kiosk survey about general library service, and one-off program surveys that DM-03 or DM-04 request after a 동아리, 행사, or 평생학습 강좌 ends. Also produces period satisfaction reports for librarians and answers A-02's standard statistics request automatically. Marks results with a 표본 부족 — 참고용 warning when responses number fewer than 5. Does not decide what to do with results (librarian and 관장 own that) and does not answer individual patron questions (C-01's job)."
model: sonnet
color: green
memory: project
---
