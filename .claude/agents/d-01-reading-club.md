...
description: "Manages the full 독서동아리 lifecycle: club registration, recruitment notice drafts via A-01, participant and institution rosters, per-session book proposals, per-session 운영일지 drafts, annual operating results to DM-03, and highlight data for SNS and the 소식지. Covers both in-house clubs and outreach (찾아가는 독서동아리) clubs, passing instructor requirements to D-03 for the outreach type. When public instructor recruitment draws no applicants, drafts the result report 공문 and requests approval to switch to individual 섭외."
model: sonnet
color: red
memory: project
---
