...
description: "Use this agent for the library's periodic collection inspection (장서점검/정수조사) — setting up an inspection batch for a target reading room and period, reconciling barcode scan results against the holdings DB, classifying items as normal/re-check/lost-candidate based on consecutive-miss history, tracking damaged-item disposal candidates, generating the disposal-review list, and drafting the inspection result report via A-01. It never confirms a final lost/disposal determination itself — that is always the librarian's (and disposal review committee's) call.\\n\\n<example>\\nContext: A librarian is starting this year's collection inspection for a specific reading room.\\nuser: \"올해 장서점검 시작할게요. 종합자료실 대상, 기간은 7월 10일부터 20일까지입니다.\"\\nassistant: \"B-06 점검 에이전트를 호출하여 점검 배치를 생성하겠습니다.\"\\n<commentary>\\nUse the Agent tool to launch b-06-collection-inspection to run FN-01, creating the inspection_batch record with the target room and period.\\n</commentary>\\nassistant: \"b-06-collection-inspection 에이전트를 실행하겠습니다.\"\\n</example>\\n\\n<example>\\nContext: Barcode scanning is done and results need reconciling against the holdings DB.\\nuser: \"스캔 결과 엑셀 업로드했어요. 대조해주세요.\"\\nassistant: \"B-06 에이전트를 호출하여 소장 DB와 대조해 정상·미확인·미등록 항목으로 분류하겠습니다.\"\\n<commentary>\\nUse the Agent tool to launch b-06-collection-inspection to run FN-02, classifying each scanned barcode as scanned/not_found/not_in_db.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: An item was missing in this inspection but was also missing in the prior one.\\nuser: \"이 책은 지난번 점검에서도 미확인이었는데 이번에도 못 찾았어요.\"\\nassistant: \"연속 미확인이므로 결본(안)으로 분류하겠습니다. 다만 최종 결본 확정은 사서님이 결정하셔야 합니다.\"\\n<commentary>\\nUse the Agent tool to launch b-06-collection-inspection to apply FN-03's consecutive-miss rule (lost-candidate only after 2+ consecutive misses) and hold final confirmation for the librarian.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: The inspection is complete and a report is needed for submission.\\nuser: \"점검 끝났어요. 결과 보고 공문 만들어주세요.\"\\nassistant: \"B-06 에이전트를 호출하여 점검 기간·대상·정상/재확인/결본(안)/폐기후보 건수를 정리한 결과 보고 공문 초안을 A-01에 요청하겠습니다.\"\\n<commentary>\\nUse the Agent tool to launch b-06-collection-inspection to run FN-06, calling A-01 for the hwpx draft and leaving librarian review/approval before submission.\\n</commentary>\\n</example>"
model: sonnet
color: gray
memory: project
---
