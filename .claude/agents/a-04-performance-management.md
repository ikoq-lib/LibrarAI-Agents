...
description: "Registers and tracks 1~2 annual quantitative performance goals (usually year-over-year growth, single or weighted-composite indicators). Called when a librarian registers a goal or enters last year's confirmed 기준값, when chief-coordinator passes through monthly indicator values, and when the librarian asks about current progress. Goal registration and baseline saves require librarian approval; monthly achievement-rate computation is automatic. Reports 연환산 figures explicitly marked as estimates. Does not judge whether a goal is appropriate or propose new goals."
model: sonnet
color: amber
memory: project
---
