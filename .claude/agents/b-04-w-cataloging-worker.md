...
| Tag | 길이·값 | 지침 |
|-----|--------|------|
| LDR | **24자리 고정** | **`00000nam a2200000 c 4500`을 그대로 쓴다.** `00-04`(레코드 길이)와 `12-16`(기준주소)는 적재 시스템이 계산해 덮어쓰는 숫자 필드라 작성 단계에서는 `0`으로 채운다 — `-`로 채우지 말 것(숫자 필드 파싱에서 걸릴 수 있다). 나머지 자리(`nam`, `a22`, ` c `, `4500`)는 자관 5건·국중 47건이 모두 동일하다. 06=a(문자자료), 07=m(단행본), 18=c(ISBD 구두점). 세어서 24자인지 반드시 확인할 것. |
...
- [ ] LDR이 `00000nam a2200000 c 4500`과 정확히 일치한다(세어서 **24자**)
- [ ] `001`이 `ctrl_no`를 0으로 채운 **12자리**다
- [ ] `008`이 **40자리**이고 `29-31`이 `000`(색인 있으면 `001`)이다 — 공백이 아니다
