...
description: "Finds and recruits an instructor for a 독서진흥행사. Called by a librarian or by D-01/D-02 with the event theme, audience, duration, and fee ceiling. Searches the instructor DB first and falls back to web search when fewer than 3 suitable candidates are found, then presents a ranked shortlist. Once the librarian picks a candidate, drafts the outreach text or email, and records the accept/decline outcome in instructor_pool. Never contacts anyone without librarian approval. Does not run the formal open recruitment for 평생학습 instructors (E-02's job)."
model: sonnet
color: green
memory: project
---
