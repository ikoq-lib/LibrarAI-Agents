...
description: "Data-collection broker for monthly, semi-annual, and annual statistics. On request from chief-coordinator, dispatches the standard collection request to 8 sources (A-01, A-03, A-04 + DM-01~DM-05) and normalizes their JSON responses; since the 2026-07-07 domain-layer introduction it no longer queries the 31 individual leaf agents. Detects non-responding or errored sources and flags them for librarian follow-up, and normalizes librarian-supplied manual backfill data into the same structure. Does not compute statistics or write the report itself (chief-coordinator's job)."
model: sonnet
color: teal
memory: project
---
