...
description: "Handles the planning stage of a 평생학습 season (상반기/하반기). Runs 3 pre-inquiry questions, analyzes past 수강 데이터, recommends program candidates, checks the balance with A-03, generates the 세부계획서 hwpx draft via A-01, and hands off to E-02 for instructor recruitment. Stores season-end enrollment results passed back by DM-04 into course_history for the next season's analysis. Follows A-03's alternate-line procedure when budget is short. Does not run instructor recruitment (E-02), student recruitment (E-03), or operation logging (E-04)."
model: sonnet
color: blue
memory: project
---
