...
with st.sidebar:
    ...
    subject_endpoint = st.text_input("주제정보 API 호출 URL", value=DEFAULT_SUBJECT_ENDPOINT, help="기본값이 국립중앙도서관 주제정보 목록 조회 URL입니다. Swagger의 Request URL에서 ? 앞부분까지 입력합니다.")
...
if st.button("주제어 테스트 조회"):
    if not public_key or not subject_endpoint.strip():
        st.warning("공공데이터포털 인증키와 주제정보 API 호출 URL을 입력하세요.")
    else:
        try:
...
if st.button("도서목록 주제어 생성", type="primary", disabled=not bool(isbns)):
    if run_aladin and not aladin_key:
        ...
        st.stop()
    if run_subject and (not public_key or not subject_endpoint.strip()):
        st.error("공공데이터포털 인증키와 주제정보 API 호출 URL을 입력하세요.")
        st.stop()
