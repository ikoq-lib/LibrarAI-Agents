...
def xml_to_dict_list(text: str) -> list[dict[str, Any]]:
    ...
    candidates = soup.find_all(["item", "row", "data", "subject", "result"])
    rows = []
    for node in candidates:
        children = node.find_all(recursive=False)
        if children:
            rows.append({child.name: child.get_text(" ", strip=True) for child in children})
    if not rows:
        rows.append({"raw": soup.get_text(" ", strip=True)[:800]})
    return rows
...
def find_records(payload: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, dict):
        return []
    keys = ["items", "item", "data", "result", "results", "list", "row", "body"]
    queue = [payload]
    while queue:
        cur = queue.pop(0)
        ...
        if isinstance(v, dict):
            ...
            for item in v:
                if isinstance(item, dict):
                    simple.append("; ".join(f"{a}:{b}" for a, b in flatten_record(item).items()))
                else:
...
def normalize_subject_records(keyword: str, records: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    for rec in records:
        ...
        preferred = pick_first(flat, ["prefLabel", "preferredTerm", "subject", "subjectName", "name", "label", "title", "우선어", "주제명"])
        uf = pick_first(flat, ["altLabel", "useFor", "uf", "비우선어"])
        bt = pick_first(flat, ["broader", "bt", "상위어"])
        nt = pick_first(flat, ["narrower", "nt", "하위어"])
        rt = pick_first(flat, ["related", "rt", "관련어"])
        uri = pick_first(flat, ["uri", "id", "identifier", "subjectId"])
        summary = " / ".join(x for x in [preferred, uf, bt, nt, rt] if x)
        rows.append({
            "조회키워드": keyword,
            "표준주제어_후보": preferred or summary or str(rec)[:120],
            "비우선어_UF": uf,
            "상위어_BT": bt,
            "하위어_NT": nt,
            "관련어_RT": rt,
            "식별자_URI": uri,
            "원문요약": summary or str(flat)[:500],
        })
    return rows
...
with st.sidebar:
    ...
    keyword_param = st.text_input("검색어 요청변수명", value="keyword", help="Swagger에서 검색어/키워드/주제어에 해당하는 요청변수명을 넣으세요. 예: keyword, kwd, searchWrd 등")
    page_param = st.text_input("페이지 요청변수명", value="pageNo")
    size_param = st.text_input("건수 요청변수명", value="numOfRows")
    response_param = st.text_input("응답형식 요청변수명", value="returnType", help="API마다 returnType, _type, resultType, apiType 등이 다를 수 있습니다. 없으면 비우세요.")
