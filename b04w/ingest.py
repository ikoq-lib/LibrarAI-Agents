...
ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "rework_note": ("재작업지시", "재작업", "rework", "rework_note"),
    "original_language": ("원작언어", "원작 언어", "원어", "original_language"),
    "kdc_final": ("확정분류", "사서확정kdc", "확정kdc", "kdc_final"),
    "toc": ("목차", "toc"),
    ...
    for seq, row in enumerate(rows[header_pos + 1:], start=1):
        ...
        books.append(
            BookInput(
                ...
                rework_note=_cell(row, index, "rework_note"),
                original_language=_cell(row, index, "original_language"),
                kdc_final=_cell(row, index, "kdc_final"),
